...
try:
	...
	print('cannot move ' + root_dir + '/output' + ' --> ' + root_dir + '/' + archive_dir_name)
	sys.exit()
except:
...
posts.sort(reverse=True) #by date descending, this helps with getting a recent post list

os.mkdir(root_dir + '/output/archive')
os.mkdir(root_dir + '/output/feed')

# write header to TOC page
with open(root_dir + '/output/archive/index.html', encoding='utf-8', mode='w') as pt:
...
with open(root_dir + '/output/feed/rss.xml', encoding='utf-8', mode='w') as ft:
	ft.write(Template(rss_top_template).substitute(date=datetime.now().strftime('%a, %d %b %Y %H:%M:%S -0400')))
...
for p in posts:
	...
	p_year_string = datetime.strptime(p.date, '%Y-%m-%d %H:%M:%S').strftime('%Y')
	
	print('Generating ' + p.name + ' ' + datetime.strptime(p.date, '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d'))
	
	if p_year_string != year_string:
		os.mkdir(root_dir + '/output/' + p_year_string)
		with open(root_dir + '/output/' + p_year_string + '/index.html', encoding='utf-8', mode='w') as nyp:
			nyp.write(top_template)
		#add footer to previous year
		if	year_string != "":
			with open(root_dir + '/output/' + year_string + '/index.html', encoding='utf-8', mode='a') as nyp:
				nyp.write(bottom_template)
	
	# create year/month archive directory if needed
	p_month_string = datetime.strptime(p.date, '%Y-%m-%d %H:%M:%S').strftime('%m')
	if p_year_string + p_month_string != year_string + month_string:
		os.mkdir(root_dir + '/output/' + p_year_string + "/" + p_month_string)
		with open(root_dir + '/output/' + p_year_string + "/" + p_month_string + '/index.html', encoding='utf-8', mode='w') as nmp:
			nmp.write(top_template)

		#add footer to previous year/month
		if	year_string != "" and month_string != "":
			with open(root_dir + '/output/' + year_string + '/' + month_string + '/index.html', encoding='utf-8', mode='a') as nyp:
				nyp.write(bottom_template)

	# create year/month/day archive directory if needed
	p_day_string = datetime.strptime(p.date, '%Y-%m-%d %H:%M:%S').strftime('%d')
	if p_year_string + p_month_string + p_day_string != year_string + month_string + day_string:
		os.mkdir(root_dir + '/output/' + p_year_string + "/" + p_month_string + "/" + p_day_string)
		with open(root_dir + '/output/' + p_year_string + "/" + p_month_string + '/' + p_day_string + '/index.html', encoding='utf-8', mode='w') as ndp:
			ndp.write(top_template)
		
		#add footer to previous year/month/day
		if	year_string != "" and month_string != "" and day_string != "":
			with open(root_dir + '/output/' + year_string + '/' + month_string + '/' + day_string + '/index.html', encoding='utf-8', mode='a') as nyp:
				nyp.write(bottom_template)
			
	permalink_rel = "/" + p_year_string + "/" + p_month_string + "/" + p_day_string + "/" + p.name
	p_date_string = datetime.strptime(p.date, '%Y-%m-%d %H:%M:%S').strftime('%b %d, %Y')

	# write to year archive
	with open(root_dir + '/output/' + p_year_string + '/index.html', encoding='utf-8', mode='a') as pt:
		pt.write(Template(post_template).substitute(permalink=permalink_rel, title=p.title, text=p.text, date=p_date_string))
		pt.write("<hr />")

	# write to year/month archive
	with open(root_dir + '/output/' + p_year_string + "/" + p_month_string + '/index.html', encoding='utf-8', mode='a') as pt:
		pt.write(Template(post_template).substitute(permalink=permalink_rel, title=p.title, text=p.text, date=p_date_string))
		pt.write("<hr />")

	# write to year/month/day archive
	with open(root_dir + '/output/' + p_year_string + "/" + p_month_string + '/' + p_day_string + '/index.html', encoding='utf-8', mode='a') as pt:
		pt.write(Template(post_template).substitute(permalink=permalink_rel, title=p.title, text=p.text, date=p_date_string))
		pt.write("<hr />")
	
	# write the post page
	os.mkdir(root_dir + '/output/' + p_year_string + "/" + p_month_string + "/" + p_day_string + "/" + p.name)
	with open(root_dir + '/output/' + p_year_string + "/" + p_month_string + '/' + p_day_string + '/' + p.name + '/index.html', encoding='utf-8', mode='a') as pt:
		pt.write(top_template)
		pt.write(Template(post_template).substitute(permalink=permalink_rel, title=p.title, text=p.text, date=p_date_string))
	...
	p_date_string_rss = datetime.strptime(p.date, '%Y-%m-%d %H:%M:%S').strftime('%a, %d %b %Y %H:%M:%S -0400')
	# RSS page to show 20 recent entries
	if processed < 20:
		with open(root_dir + '/output/feed/rss.xml', encoding='utf-8', mode='a') as ft:
			ft.write(Template(rss_post_template).substitute(permalink=base_url + permalink_rel, title=p.title, text=p.text, date=p_date_string_rss))

	p_date_string_short = datetime.strptime(p.date, '%Y-%m-%d %H:%M:%S').strftime('%Y.%m')
	#add to TOC page
	with open(root_dir + '/output/archive/index.html', encoding='utf-8', mode='a') as pt:
		pt.write(Template(title_only_template).substitute(permalink=permalink_rel, title=p.title, date=p_date_string_short))
...
# add footer to RSS
with open(root_dir + '/output/feed/rss.xml', encoding='utf-8', mode='a') as pt:
	pt.write(rss_bottom_template)
	
# Contact page
os.mkdir(root_dir + '/output/contact')
with open(root_dir + '/output/contact/index.html', encoding='utf-8', mode='w') as cp:
...
with open(root_dir + '/output/others/index.html', encoding='utf-8', mode='w') as op:
